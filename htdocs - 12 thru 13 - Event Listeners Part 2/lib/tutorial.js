...
function onMarksSelection()
{
    ...
    if (sheetName =="Visits"){
		// Simple scenario - Visits Worksheet
	...
	else if (sheetName == "BigMap"){
		// reference the floating VisitsByTime worksheet inside the dashboard and get marks
		nestedSheet = viz.getWorkbook().getActiveSheet().getWorksheets().get("VisitByTime");
		nestedSheet.getSelectedMarksAsync().then(function(result){
			console.log('Marks Read');
			// If more than 5 marks selected, user is interested in line chart. Nav to it.
			if (result.length > 5) {viz.getWorkbook().activateSheetAsync('BigChart');}
		});
	}
}
